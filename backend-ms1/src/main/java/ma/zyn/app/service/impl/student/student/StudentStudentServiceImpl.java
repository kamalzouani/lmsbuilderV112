...
public class StudentStudentServiceImpl implements StudentStudentService {
    ...
    public Student findWithAssociatedLists(Long id){
        ...
        if(result!=null && result.getId() != null) {
            result.setCourses(courseService.findByStudentId(id));
        }
        return result;
    }
    ...
    if(student !=null && student.getId() != null){
        List<List<Course>> resultCourses= courseService.getToBeSavedAndToBeDeleted(courseService.findByStudentId(student.getId()),student.getCourses());
            courseService.delete(resultCourses.get(1));
        emptyIfNull(resultCourses.get(0)).forEach(e -> e.setStudent(student));
