...
@Component({
  selector: 'app-enrollment-list-admin',
  templateUrl: './enrollment-list-admin.component.html'
})
export class EnrollmentListAdminComponent implements OnInit {

    protected fileName = 'Enrollment';
    ...
    public findPaginatedByCriteria() {
        this.service.findPaginatedByCriteria(this.criteria).subscribe(paginatedItems => {
            ...
            this.selections = new Array<EnrollmentDto>();
    ...
    public async edit(dto: EnrollmentDto) {
    ...
    public async view(dto: EnrollmentDto) {
        this.service.findByIdWithAssociatedList(dto).subscribe(res => {
            this.item = res;
    ...
    public async delete(dto: EnrollmentDto) {
    ...
    public async duplicate(dto: EnrollmentDto) {
    ...
    public exportPdf(dto: EnrollmentDto): void {
...
// add
    ...
    public initCol() {
        this.cols = [
            ...
            {field: 'student?.email', header: 'Student'},
            {field: 'course?.id', header: 'Course'},
        ];
    }


    public async loadStudent(){
        this.studentService.findAllOptimized().subscribe(students => this.students = students, error => console.log(error))
    }
    public async loadCourse(){
        this.courseService.findAll().subscribe(courses => this.courses = courses, error => console.log(error))
    }


	public initDuplicate(res: EnrollmentDto) {
	}
        ...
        this.service.findByCriteria(this.criteria).subscribe(
            (allItems) =>{
                this.exportData = allItems.map(e => {
					return {
						'Enrollment date': this.datePipe.transform(e.enrollmentDate , 'dd/MM/yyyy hh:mm'),
            ...
            this.criteriaData = [{
                'Enrollment date Min': this.criteria.enrollmentDateFrom ? this.datePipe.transform(this.criteria.enrollmentDateFrom , this.dateFormat) : environment.emptyForExport ,
                'Enrollment date Max': this.criteria.enrollmentDateTo ? this.datePipe.transform(this.criteria.enrollmentDateTo , this.dateFormat) : environment.emptyForExport ,
            ...
            //'Student': this.criteria.student?.email ? this.criteria.student?.email : environment.emptyForExport ,
            //'Course': this.criteria.course?.id ? this.criteria.course?.id : environment.emptyForExport ,
            }];
			}
    ...
    get selections(): Array<EnrollmentDto> {
        return this.service.selections;
    }

    set selections(value: Array<EnrollmentDto>) {
